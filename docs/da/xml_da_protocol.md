---
title: XML DA Protocol
---

## General Info

The XML DA Protocol (also known as V6 DA Protocol) is the latest communication protocol used by [Download Agent](download_agent.mdx) in MediaTek devices to interact with the device during Download Mode.
It is built upon the previous [XFlash DA Protocol (V5)](xflash_da_protocol.md), while introducing significant changes to the communication layer.
...
Being built upon the [XFlash DA Protocol](xflash_da_protocol.md), each packet is similarly preceded by a 12 bytes header with the following format:
