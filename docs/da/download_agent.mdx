...
In Mediatek devices, a Download Agent (DA) is a special file containg code that is sent to the device through [BROM](../modes/brom.md) or [Preloader](../modes/preloader.mdx) via serial communication.

The DA allows through specialized tools (like [SP Flash Tool](../dev/tools.md), [mtkclient](../dev/mtkclient.md) and [Penumbra](../dev/penumbra.md)) to perform a variety of specified actions on the device, most notably:
* Reading and writing flash
* Read and Write RPMB (Through [DA Extensions](da_extensions.md) or a a specialised DA)
* Get info on the device (Chip ID, MEID, OTP ver...)
* Read and Write e-fuses
* Unlock or relock [Seccfg](../dev/seccfg.mdx) (Through [DA Extensions](da_extensions.md))

A Download Agent operates under three different protocols (ordered from oldest to newest):

* Legacy (V3)
* [XFlash DA Protocol (V5)](xflash_da_protocol.md)
* [XML DA Protocol (V6)](xml_da_protocol.md) 
...
Exploits like [Kamakiri](https://penumbra.itssho.my/Mediatek/Exploits/Kamakiri) allows to temporarily disable phone security protections (SLA, DAA and SBC) to be able to load an arbitrary patched DA1.
...
[Carbonara](../info/exploits.mdx#preloader---carbonara) abuses a flawed implementation of this cmd to overwrite the DA2 hash contained in DA1, disrupting this way the whole root of trust and allowing to load an arbitrary payload.
...
Before some still unknown time in 2024, DA2 also implemented the `boot_to` cmd, which allowed [DA Extensions](da_extensions.md) to be arbitrary loaded with a patched second stage.


## Download Agent Structure

<img
  src="../../assets/da/da_v5.png"
  alt="DA V5 structure"
/>
*DA V5 structure*

Between Legacy V3, [XFlash DA Protocol](xflash_da_protocol.md) and [XML DA Protocol](xml_da_protocol.md), the structure varies with small differences, but generally it's quite consistent.
...
| DA Entries           | `0x6C-0x??` | Each DA entries contains metadata on the DA and their regions. On Legacy, the size of each DA entry is `0xD8`, while on XFlash and XML it's `0xDC`    |
